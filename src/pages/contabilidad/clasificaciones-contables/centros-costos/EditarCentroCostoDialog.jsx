...
const EditarCentroCostoDialog = ({ open, onClose, centroCosto, onCentroCostoActualizado }) => {
  ...
  // Precargar datos del centro de costo cuando se abre el diálogo
  useEffect(() => {
    if (open && centroCosto) {
      setValue('sub_centro_costo', centroCosto.sub_centro_costo || '');
      setValue('centro_costo_macro', centroCosto.centro_costo_macro || '');
      setValue('descripcion_cc', centroCosto.descripcion_cc || '');
      setMostrarInputNuevo(false);
      setValorNuevoMacro('');
    }
  }, [open, centroCosto, setValue]);
  ...
  const onSubmit = async (data) => {
    setLoading(true);
    try {
      const response = await fetch(`http://localhost:5000/api/centros-costos/${centroCosto.id_centro_costo}`, {
        method: 'PUT',
        headers: {
      ...
      if (response.ok) {
        ...
        onCentroCostoActualizado?.();
        onClose();
      } else {
        throw new Error('Error al actualizar el centro de costo');
      }
    } catch (error) {
      toast({
        title: "Error",
        description: "No se pudo actualizar el centro de costo",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-gray-500 mt-1">
                Modifique la información del centro de costo ID: {centroCosto.id_centro_costo}
              </p>
            </div>
          </div>
        </DialogHeader>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="p-3 bg-blue-50 rounded-lg border border-blue-200">
                <div className="flex items-center justify-between">
                  <span className="text-sm font-medium text-blue-900">ID del Centro</span>
                  <span className="text-xs bg-blue-600 text-white px-2 py-1 rounded font-mono">
                    {centroCosto.id_centro_costo}
                  </span>
                </div>
                <p className="text-xs text-blue-700 mt-1">Identificador único del centro</p>
              </div>
              ...
              <div className="p-3 bg-green-50 rounded-lg border border-green-200">
                ...
                <p className="text-xs text-green-700 mt-1">El centro se mantendrá activo</p>
              </div>

              <div className="p-3 bg-purple-50 rounded-lg border border-purple-200">
                <div className="flex items-center justify-between">
                  <span className="text-sm font-medium text-purple-900">Tipo</span>
                  <span className="text-xs bg-purple-600 text-white px-2 py-1 rounded">Centro de Costo</span>
                </div>
                <p className="text-xs text-purple-700 mt-1">Para control de costos</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white border rounded-lg p-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="p-3 bg-orange-50 rounded-lg border border-orange-200">
                <div className="flex items-center justify-between">
                  <span className="text-sm font-medium text-orange-900">Nivel Actual</span>
                  <span className="text-xs bg-orange-600 text-white px-2 py-1 rounded">
                    {centroCosto.centro_costo_macro ? 'Sub-centro' : 'Centro Principal'}
                  </span>
                </div>
                <p className="text-xs text-orange-700 mt-1">Posición en la jerarquía organizacional</p>
              </div>
              ...
              <div className="p-3 bg-indigo-50 rounded-lg border border-indigo-200">
                ...
                <p className="text-xs text-indigo-700 mt-1">Organización de centros de costo</p>
              </div>
            </div>
          </div>
...
export default EditarCentroCostoDialog; 