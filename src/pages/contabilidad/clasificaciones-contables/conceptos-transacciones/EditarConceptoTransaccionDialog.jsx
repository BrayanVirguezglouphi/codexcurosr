...
const EditarConceptoTransaccionDialog = ({ open, onOpenChange, concepto, onConceptoActualizado }) => {
  ...
  // Cargar datos y precargar formulario
  useEffect(() => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (response.ok) {
        toast({
          title: "Éxito",
          description: "Concepto de transacción actualizado correctamente",
        });
        
        onConceptoActualizado?.();
        onOpenChange(false);
      } else {
        throw new Error('Error al actualizar el concepto de transacción');
      }
    } catch (error) {
      toast({
        title: "Error",
        description: "No se pudo actualizar el concepto de transacción",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              <DialogTitle className="text-2xl font-bold text-gray-900">
                Editar Concepto de Transacción
              </DialogTitle>
              <p className="text-sm text-gray-500 mt-1">
                Modifique la información del concepto de transacción ID: {concepto.id_concepto}
              </p>
            </div>
          </div>
        </DialogHeader>
          ...
          <div className="bg-white border rounded-lg p-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="p-3 bg-blue-50 rounded-lg border border-blue-200">
                <div className="flex items-center justify-between">
                  <span className="text-sm font-medium text-blue-900">ID del Concepto</span>
                  <span className="text-xs bg-blue-600 text-white px-2 py-1 rounded font-mono">
                    {concepto.id_concepto}
                  </span>
                </div>
                <p className="text-xs text-blue-700 mt-1">Identificador único del concepto</p>
              </div>
              ...
              <div className="p-3 bg-green-50 rounded-lg border border-green-200">
                ...
                <p className="text-xs text-green-700 mt-1">El concepto se mantendrá activo</p>
              </div>
            </div>
          </div>
...
export default EditarConceptoTransaccionDialog; 