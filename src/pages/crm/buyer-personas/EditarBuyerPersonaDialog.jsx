...
const EditarBuyerPersonaDialog = ({ open, onClose, buyerPersona, onBuyerPersonaActualizado }) => {
  ...
  // Cargar datos de la buyer persona cuando cambie
  useEffect(() => {
    if (buyerPersona && open) {
      console.log('Cargando datos de la buyer persona:', buyerPersona);
      
      reset({
        buyer: buyerPersona.buyer || '',
        background: buyerPersona.background || '',
        metas: buyerPersona.metas || '',
        retos: buyerPersona.retos || '',
        identificadores: buyerPersona.identificadores || '',
        objeciones_comunes: buyerPersona.objeciones_comunes || '',
        conductas_compra: buyerPersona.conductas_compra || '',
        que_podemos_hacer: buyerPersona.que_podemos_hacer || '',
        posible_mensaje: buyerPersona.posible_mensaje || '',
        que_oye: buyerPersona.que_oye || '',
        que_piensa_siente: buyerPersona.que_piensa_siente || '',
        que_ve: buyerPersona.que_ve || '',
        que_dice_hace: buyerPersona.que_dice_hace || '',
        esfuerzos: buyerPersona.esfuerzos || '',
        resultados_valor: buyerPersona.resultados_valor || '',
        observaciones: buyerPersona.observaciones || '',
        url_file: buyerPersona.url_file || ''
      });
    }
  }, [buyerPersona, open, reset]);
  ...
  const onSubmit = async (data) => {
    try {
      ...
      console.log('Actualizando buyer persona:', buyerPersona.id_buyer, formattedData);
      
      const buyerPersonaActualizada = await apiCall(`/api/crm/buyer-personas/${buyerPersona.id_buyer}`, {
        method: 'PUT',
        headers: {
      ...
      onBuyerPersonaActualizado?.(buyerPersonaActualizada);
      onClose();
    } catch (error) {
      console.error('Error al actualizar buyer persona:', error);
      toast({
        title: "Error",
        description: error.message || "Error al actualizar la buyer persona",
        variant: "destructive"
      });
    }
  };
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="max-w-6xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Modifique la información de la buyer persona: {buyerPersona.buyer}
          </DialogDescription>
        </DialogHeader>
...
export default EditarBuyerPersonaDialog; 