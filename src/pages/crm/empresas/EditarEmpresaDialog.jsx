...
const EditarEmpresaDialog = ({ open, onClose, empresa, onEmpresaActualizada }) => {
  ...
  // Cargar datos de la empresa cuando cambie
  useEffect(() => {
    if (empresa && open) {
      console.log('Cargando datos de la empresa:', empresa);
      
      reset({
        empresa: empresa.empresa || '',
        id_mercado: empresa.id_mercado?.toString() || '',
        id_pais: empresa.id_pais?.toString() || '',
        tamano_empleados: empresa.tamano_empleados || '',
  ...
  const onSubmit = async (data) => {
    try {
      ...
      console.log('Actualizando empresa:', empresa.id_empresa, formattedData);
      
      const empresaActualizada = await apiCall(`/api/crm/empresas/${empresa.id_empresa}`, {
        method: 'PUT',
        headers: {
      ...
      onEmpresaActualizada?.(empresaActualizada);
      onClose();
    } catch (error) {
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Modifique la información de la empresa: {empresa.empresa}
          </DialogDescription>
        </DialogHeader>
...
export default EditarEmpresaDialog; 